...
class SharedOffloadRegion:
    ...
    def __init__(
        ...
        # Sandboxed/constrained kernels (e.g. Kata containers) reject
        # MADV_POPULATE_WRITE on tmpfs with EINVAL. Populate is only a
        # prefault hint — demand paging still works — so degrade gracefully
        # instead of failing engine startup.
        def _madvise_populate(offset: int, length: int) -> None:
            try:
                self.mmap_obj.madvise(_MADV_POPULATE_WRITE, offset, length)
            except OSError as exc:
                logger.warning(
                    "MADV_POPULATE_WRITE failed (%s); continuing with "
                    "demand paging (constrained kernel/tmpfs)",
                    exc,
                )
        ...
        if rank is not None:
            ...
            for block in range(num_blocks):
                ...
                _madvise_populate(aligned_offset, aligned_length)
            logger.debug(
            ...
            _t0 = time.perf_counter()
            _madvise_populate(0, self.total_size_bytes)
            logger.debug(
